...
for (( round=1; round<=20; round++ )); do
  [ $DEBUG -eq 1 ] && echo "<---------------- Round[$round] ---------------->"
  echo "<---------------- Round[$round] ---------------->" >> $LOGFILE
  for (( monkey=0; monkey<=num_monkeys-1; monkey++ )); do
    this_operation=${operations[$monkey]}
    [ $DEBUG -eq 1 ] && echo "<~~~~~~~~~~~~~~~~ Monkey[${monkey}][${this_operation}] ~~~~~~~~~~~~~~~>"
    echo "<~~~~~~~~~~~~~~~~ Monkey[${monkey}][${this_operation}] ~~~~~~~~~~~~~~~>" >> $LOGFILE

    for (( tmonkey=0; tmonkey<=num_monkeys-1; tmonkey++ )); do
      [ $DEBUG -eq 1 ] && echo "Befor[${round}][${tmonkey}]: ${items[$tmonkey]}"
      # echo "Befor[${round}][${tmonkey}]: ${items[$tmonkey]}" >> $LOGFILE
    done
    for item in ${items[$monkey]}; do
      new_item=0
      [ $DEBUG -eq 1 ] && echo " item: $item $val $this_operation"
      # echo " item: $item $val $this_operation" >> $LOGFILE
      if [ "$this_operation" == "*old" ]; then
        new_item=$(((item*item)/div))
        [ $DEBUG -eq 1 ] && echo "item[0]: (${item}*${item}/${div}) = $new_item"
        # echo "item[0]: (${item}*${item}/${div}) = $new_item" >> $LOGFILE
        inspections[$monkey]=$((inspections[monkey]+1))
      else
        if [ "${this_operation:0:1}" == "*" ]; then
          val=${this_operation:1}
          new_item=$(((item*val)/div))
          [ $DEBUG -eq 1 ] && echo "item[1]: (${item}*${val}/${div}) = $new_item"
          # echo "item[1]: (${item}*${val}/${div}) = $new_item" >> $LOGFILE
          inspections[$monkey]=$((inspections[monkey]+1))
        else
          val=${this_operation:1}
          new_item=$(((item+val)/div))
          [ $DEBUG -eq 1 ] && echo "item[2]: (${item}+${val}/${div}) = $new_item"
          # echo "item[2]: (${item}+${val}/${div}) = $new_item" >> $LOGFILE
          inspections[$monkey]=$((inspections[monkey]+1))
        fi
      fi
      if [ "$(expr $new_item % ${tests[$monkey]})" -eq 0 ]; then
        throw_to=${ttrue[$monkey]}
        [ $DEBUG -eq 1 ] && echo " > throw to $throw_to"
        # echo " > throw to $throw_to" >> $LOGFILE
      else
    ...
    for (( tmonkey=0; tmonkey<=num_monkeys-1; tmonkey++ )); do
      ...
      [ $DEBUG -eq 1 ] && echo "After[${round}][${tmonkey}]: ${items[$tmonkey]}"
      echo "After[${round}][${tmonkey}]: ${items[$tmonkey]}" >> $LOGFILE
    done
    # sleep 1
  done
done
