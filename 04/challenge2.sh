...
for line in "${values[@]}"; do
  ...
  if [[ $elf1low -ge $elf2low ]] && [[ $elf1low -le $elf2high ]]; then
    total=$((total+1))
  elif [[ $elf2low -ge $elf1low ]] && [[ $elf2low -le $elf1high ]]; then
    total=$((total+1))
  elif [[ $elf1high -eq $elf2high ]] || [[ $elf1low -eq $elf2low ]] || [[ $elf1high -eq $elf2low ]] || [[ $elf1low -eq $elf2low ]]; then
    total=$((total+1))
  fi
done
echo >> $LOGFILE
